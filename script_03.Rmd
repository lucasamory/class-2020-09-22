...
sep_old <- 
  ...
  filter(u_grad > 10) %>%
  group_by(title, id, department, name) %>%
...
* Throw out columns we don't care about
* Define terms of potential outcomes


After we have a model, Temperance suggests we should:

* Acknowledge that this year is different from others, which would affect external validity, so it can't be generalized.
* Since a large amount of the population at Harvard is taking a leave of absence, and they were significant enough to potentially sway the data, we need to acknowledge that they
